...
class HideNewsTest {
    ...
    @Test
    @DisplayName("뉴스가 성공적으로 숨김 처리되어야 한다")
    void hideNews_success() {
        // given
        Long newsId = 1L;
        String authorId = "authorId";
        NewsHideCommand command = new NewsHideCommand(newsId, authorId);
        ...
        // when
        NewsHideResult result = hideNews.hideNews(command);

        // then
        verify(newsStore).store(news);
        assertThat(news.getStatus()).isEqualTo(NewsStatus.HIDDEN);
